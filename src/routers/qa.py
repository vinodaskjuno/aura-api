...
def _runner_identity(request: Request) -> tuple[str, dict]:
    ...
    # A rejected poll is anonymous by definition, so it can never reach a runner row.
    # Counting it is the only way the UI can distinguish "nobody is running an agent"
    # from "an agent is running and its key is being refused" — which read identically
    # for four days while a rotated key silently 401'd every five seconds.
    try:
        credential = extract_credential(request)
    except HTTPException:
        queue.record_unauthorized()
        raise
    try:
        user = resolve_credential(credential)
    except HTTPException:
        queue.record_unauthorized(str(credential)[-4:])
        raise
...
def _no_workspace_detail(project_id: str) -> str:
    ...
    Reuses `service._no_working_copy` so the API and the runner give ONE explanation of
    the same condition rather than drifting into two.
    """
    from src.qatest import appserver, service

    _root, checked = appserver.locate(project_id)
...
def enqueue_run(body: EnqueueRequest,
    ...
    Refuses up front when the run provably cannot succeed. Without an `app_url` the
    runner needs a working copy, and the only one it can get is the copy this server
    ships it — so if this server has none, the run is already doomed. It used to be
    accepted anyway, sit in the queue, get claimed, and fail on the runner minutes later
    with a filesystem path from a machine the reader has never seen. Answering at the
    button press is the same information, delivered where it can still be acted on.
    """
    from src.qatest import queue, workspace

    if not body.app_url and workspace.local_root(body.project_id) is None:
        raise HTTPException(status_code=409, detail=_no_workspace_detail(body.project_id))
...
def get_run_cost(run_id: str, projectId: str = Query(...),
                 _: dict = Depends(require_permission("qa_workspace"))):
    """What this run spent on model calls.

    Today the honest answer is nothing. A run's plan is derived from the knowledge graph
    rather than generated by a model — a deliberate choice recorded in qatest/plan.py,
    because a generated plan is neither reproducible nor free — and execution is
    Playwright, HTTP and file checks. So `calls` is 0 and the UI says so in words rather
    than rendering $0.00, which would claim a measurement that was never taken.

    It is wired anyway. `exploratory` is already carried end to end on the run row, and
    the first thing it will do when implemented is call a model; without this the spend
    would land in a pool nobody could attribute. The agent exports AURA_TEST_RUN_ID into
    the run's environment and the gateway records whatever arrives in X-Aura-Test-Run-Id.

    Reads the projectId GSI rather than scanning: usage rows are the highest-volume
    table here and a per-run scan would be charged for every row it discards.
    """
    ...
    try:
        rows = db.query_items("token-usage", "projectId", projectId,
                              index_name="projectId-timestamp-index", limit=1000)
    except Exception as exc:                                  # noqa: BLE001
        # A deployment whose table predates the index. Report the absence rather than
        # an authoritative zero — they mean very different things.
        log.warning("qa: run cost unavailable for %s: %s", run_id, exc)
        raise HTTPException(
            status_code=503,
            detail="Usage history cannot be read: the token-usage projectId index is "
                   "missing in this environment.")

    for row in rows or []:
        if str(row.get("testRunId") or "") != run_id:
            continue
        calls += 1
        for key in totals:
            totals[key] += int(row.get(key) or 0)
        # Prefer the cost recorded at the time: prices change, and re-pricing an old
        # row would quietly restate history.
        try:
            cost += float(row.get("cost") or 0)
        except (TypeError, ValueError):
            pass
        model = str(row.get("model") or "unknown")
        seen = by_model.setdefault(model, {"model": model, "calls": 0, "cost": 0.0,
                                           "inputTokens": 0, "outputTokens": 0})
        seen["calls"] += 1
        seen["inputTokens"] += int(row.get("inputTokens") or 0)
        seen["outputTokens"] += int(row.get("outputTokens") or 0)
        try:
            seen["cost"] += float(row.get("cost") or 0)
        except (TypeError, ValueError):
            pass

    return {"runId": run_id, "projectId": projectId, "calls": calls,
            **totals,
            "totalTokens": sum(totals.values()),
            "costUsd": round(cost, 6),
            "byModel": sorted(by_model.values(), key=lambda m: -m["cost"])}
...
def list_runners(user: dict = Depends(require_permission("qa_workspace"))):
    ...
    return {"runners": queue.list_runner_state(),
            ...
            # Present only when a poll was refused recently. The Runner tab uses it to
            # explain an empty list instead of leaving the reader to guess.
            "unauthorized": queue.unauthorized_recently(),
